...
function RegisterComponent(){
    ...
    const register = async (e) =>{
        e.preventDefault();
        let form_data = {email, name, password};
        try {
            const response = await axios.post("http://localhost:5000/auth/register", form_data);
            ...
            <div className="card-body">
                <form onSubmit={register}>
                    ...
                    <div className="form-group mt-2">
                        ...
                        <Link to="/login" className="btn btn-outline-success w-100 mt-2" style={{float:"right"}}>Giriş Yap</Link>
                    </div>
                </form>
            </div>
        </div>
